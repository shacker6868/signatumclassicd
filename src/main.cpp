...
#if defined(NDEBUG)
# error "Signatumclassic cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Signatumclassic Signed Message:\n";
...
bool AreInputsStandard(const CTransaction& tx, const MapPrevTx& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (whichType == TX_SCRIPTHASH)
        {
            if (stack.empty())
                return false;
            CScript susigcript(stack.back().begin(), stack.back().end());
            vector<vector<unsigned char> > vSolutions2;
            txnouttype whichType2;
            if (Solver(susigcript, whichType2, vSolutions2))
            {
            ...
            else
            {
                ...
                unsigned int sigops = susigcript.GetSigOpCount(true);
...
{
    RenameThread("signatumclassic-loadblk");
