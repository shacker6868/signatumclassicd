...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("signatumclassic"))
        return false;
...
{
    // Convert signatumclassic:// to signatumclassic:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("signatumclassic://"))
    {
        uri.replace(0, 12, "signatumclassic:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Signatumclassic.lnk";
}
...
{
    return GetAutostartDir() / "signatumclassic.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=Signatumclassic\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Signatumclassic-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  signatumclassic-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Signatumclassic-Qt"));
